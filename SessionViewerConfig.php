...
class SessionViewerConfig extends ModuleConfig {
  ...
  public function getInputfields() {
    ...
    $field->name = 'serialize_handler';
    $field->value = '<strong>'.__('Serialize handler').'</strong><p>'.__('Only the following serialize handlers are supported: php, php_binary, php_serialize.').'</p><p>'.__('Current serialize handler').': '.ini_get('session.serialize_handler').'</p>';
    ...
    $field->label = __('nice_r()');
    $field->label2 = __('Enable');
    $field->description = __('Uses nice_r() to display the session data nicer.');
